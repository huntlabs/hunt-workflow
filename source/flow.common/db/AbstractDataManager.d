...
//
//            if ((cachedObjects !is null && cachedObjects.size() > 0) || getManagedEntitySubClasses() !is null) {
//
...
//
//                // Cache entities
//                if (cachedObjects !is null && cachedEntityMatcher !is null) {
...
//                }
//
//                if (getManagedEntitySubClasses() !is null && cachedEntityMatcher !is null) {
...
//                        if (subclassCachedObjects !is null) {
...
//
//        List<EntityImpl> result = new ArrayList<>(cachedObjects !is null ? cachedObjects.size() : 1);
//        if (cachedObjects !is null && entityMatcher !is null) {
//            for (CachedEntity cachedObject : cachedObjects) {
//                EntityImpl cachedEntity = (EntityImpl) cachedObject.getEntity();
...
//        }
//
//        if (getManagedEntitySubClasses() !is null && entityMatcher !is null) {
//            for (Class<? extends EntityImpl> entitySubClass : getManagedEntitySubClasses()) {
//                Collection<CachedEntity> subclassCachedObjects = getEntityCache().findInCacheAsCachedObjects(entitySubClass);
//                if (subclassCachedObjects !is null) {
//                    for (CachedEntity subclassCachedObject : subclassCachedObjects) {
...
//        return executionEntityClass !is null && dbSqlSession.isEntityInserted(executionEntityClass, entityId);
//    }
//
//}
